  ...
  <popup v-model="showModel">
    <div class="IDO-model-wrapper">
      <div class="IDO-model-content">
        <div class="BSCAddress">Polygon {{ t('message.index.IDOModel.Address') }}</div>
        ...
        <div class="banlace">
          <div class="title">
            {{ t('message.index.IDOModel.Banlace', { token: 'HQT' }) }}
          </div>
          <div class="HQT-banlace">
            <i class="el-icon-loading" v-if="isLoadingHQT"></i>
            {{ HQTBanlace ? amountToDecimal(formatAmount(HQTBanlace)) : '-' }}
          </div>
          <div class="title">
        ...
        <div class="swap-number">
          <div class="tips">
            <div v-html="t('message.index.IDOModel.swapQuantity', { token: 'ACY', min: '200' })" />
        ...
        <div class="ACY-banlace">
          <span>{{ t('message.index.IDOModel.Banlace', { token: 'ACY' }) }}:</span>
          <span class="amount">
            {{ ACYBanlaceAmount ? amountToDecimal(formatAmount(ACYBanlaceAmount)) : '-' }}
          </span>
        </div>
        <div class="swap">
          <div class="label">
          ...
          <div class="amounts">
            <div class="amount">
              <span>{{
                parsedStablecoinAndHQTAmounts?.HQTAmount
                  ? amountToDecimal(
                      formatAmount(parsedStablecoinAndHQTAmounts.HQTAmount.toString())
                    )
                  : '-'
              }}</span>
              <span class="symbol">HQT</span>
            </div>
            <div class="amount">
              <span>
                {{
        ...
        <div class="output">
          ...
          <div class="amounts">
            <div class="amount">
              ...
              <span class="symbol">ACY</span>
            </div>
          </div>
        </div>
        <div class="approves" v-if="isConnected && (!isStablecoinApproved || !isHQTApproved)">
          <el-button
            round
          ...
          <el-button
            round
            @click="onApprove(HQT_TOKEN_ADDRESS, true)"
            :disabled="isHQTApproved || !isConnected"
            :loading="isLoadingHQTApprove"
            type="primary"
            size="medium"
          >
            <template v-if="isHQTApproved">{{ t('message.index.IDOModel.approved') }}</template>
            <template v-else>{{ t('message.index.IDOModel.approve') }}</template>
            HQT
          </el-button>
        </div>
        <div class="buttons" v-else>
          <el-button
            ...
            size="medium"
          >
            {{ t('message.index.IDOModel.swapToken', { token: 'ACY' }) }}
          </el-button>
        </div>
        <div class="bottom-tips" v-if="isInsufficientStablecoinBanlace || isInsufficientHQTBanlace">
          {{ isInsufficientStablecoinBanlace ? stablecoinLabels[stablecoin] : 'HQT' }}
...
const ACY_RATE = JSBI.BigInt(200)
const STABLECOIN_RATE = JSBI.BigInt(40)
...
export default defineComponent({
  ...
  setup(_, { expose }) {
    ...
    const maxSwapQuantity = computed(() => {
      if (HQTBanlace.value === undefined) {
        return undefined
      }
      return JSBI.multiply(ACY_RATE, HQTBanlace.value).toString()
    })
    const parsedStablecoinAndHQTAmounts = computed(() => {
      if (parsedSwapQuantity.value === undefined) {
        return undefined
      }
      //200ACY = 1HQT + 5(USDT|BUSD)
      const stablecoinAmount = JSBI.divide(parsedSwapQuantity.value, STABLECOIN_RATE)
      const HQTAmount = JSBI.divide(parsedSwapQuantity.value, ACY_RATE)
      // if (JSBI.greaterThan(JSBI.remainder(HQTAmount, BIG_ONE), ZERO)) {
      //   return undefined
      // }
      return {
        stablecoinAmount,
    ...
    const isInsufficientStablecoinBanlace = computed(() => {
      if (!parsedStablecoinAndHQTAmounts.value || !stablecoinBanlace.value) {
        return false
      }
      const { stablecoinAmount } = parsedStablecoinAndHQTAmounts.value
      return JSBI.lessThan(stablecoinBanlace.value, stablecoinAmount)
    })
    const isInsufficientHQTBanlace = computed(() => {
      if (!parsedStablecoinAndHQTAmounts.value || !HQTBanlace.value) {
        return false
      }
      const { HQTAmount } = parsedStablecoinAndHQTAmounts.value
      return JSBI.lessThan(HQTBanlace.value, HQTAmount)
    })
    const swapButtonDisabled = computed(() => {
      if (
        !isConnected.value ||
        !parsedStablecoinAndHQTAmounts.value ||
        !HQTBanlace.value ||
        !stablecoinBanlace.value
      ) {
        return true
      }
      if (JSBI.equal(ZERO, HQTBanlace.value) || JSBI.equal(ZERO, stablecoinBanlace.value)) {
        return true
      }
      const { HQTAmount } = parsedStablecoinAndHQTAmounts.value
      if (JSBI.greaterThan(JSBI.remainder(HQTAmount, BIG_ONE), ZERO)) {
      ...
      if (isInsufficientStablecoinBanlace.value || isInsufficientHQTBanlace.value) {
        return true
      }
      return false
    })
    ...
    const _getHQTBanlace = async () => {
      if (!userAccount.value || !curChainId.value) {
        return
      }
      isLoadingHQT.value = true
      const banlace = await getBalance(HQT_TOKEN_ADDRESS, userAccount.value, curChainId.value)
      HQTBanlace.value = JSBI.BigInt(banlace)
      isLoadingHQT.value = false
    }
    ...
    const _getACYBanlaceAmount = async () => {
      if (!userAccount.value || !curChainId.value) {
        return
      }
      const banlace = await getBalance(ACY_ADDRESS, userAccount.value, curChainId.value)
      ACYBanlaceAmount.value = JSBI.BigInt(banlace)
    }
    const _getHQTApproved = async () => {
      if (!userAccount.value || !curChainId.value) {
        return
      }
      isLoadingHQTApprove.value = true
      isHQTApproved.value = await getApproved(
        HQT_TOKEN_ADDRESS,
        ACY_IOD_SWAP_ADDRESS,
        userAccount.value,
        curChainId.value
      )
      isLoadingHQTApprove.value = false
    }
    ...
    const onDisconnect = () => {
      ...
      HQTBanlace.value = undefined
      stablecoinBanlace.value = undefined
      isHQTApproved.value = false
      isStablecoinApproved.value = false
    }
    const onApprove = async (tokenAddress: string, isApproveHQT: boolean) => {
      if (!userAccount.value) {
        return
      }
      isApproveHQT ? (isLoadingHQTApprove.value = true) : (isLoadingStablecoinApprove.value = true)
      const hasApproved = await approve(tokenAddress, ACY_IOD_SWAP_ADDRESS, userAccount.value)
      if (hasApproved) {
        ElMessage.success(t('message.index.IDOModel.approveSuccess'))
        isApproveHQT ? (isHQTApproved.value = true) : (isStablecoinApproved.value = true)
      } else {
    ...
    const onSwap = async () => {
      if (parsedStablecoinAndHQTAmounts.value === undefined) {
        return
      }
      const { stablecoinAmount } = parsedStablecoinAndHQTAmounts.value
    ...
    return {
      ...
      baseSwapQuantity,
      maxSwapQuantity,
      parsedStablecoinAndHQTAmounts,
      parsedSwapQuantity,

      isInsufficientStablecoinBanlace,
      isInsufficientHQTBanlace,
